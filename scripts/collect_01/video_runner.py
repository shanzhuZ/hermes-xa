"""后台视频 runner：下载+抽帧+分析，并回写 step6_video_* 状态。

由 video_job 以独立进程拉起，勿在 Hook 内同步调用。
"""
...
# scripts/ 与 video2frame-mcp 入 path
...
logger = logging.getLogger("collect_01.video_runner")


def _assert_runtime_deps() -> None:
    """尽早暴露解释器/依赖问题（避免 PIL 被 venv 污染后难查）。"""
    logger.info("video_runner python=%s", sys.executable)
    try:
...
def _mark_step(task_id: str, platform: str, status: str, message: str, payload: Optional[Dict[str, Any]] = None) -> None:
    from collect_01.phases import video_step_key
    ...
    *,
    ...
    def _work() -> None:
        try:
            ...
            logger.exception("video_runner 异常")
            error_holder["error"] = str(exc)

    th = threading.Thread(target=_work, name=f"video-{platform}", daemon=True)
    th.start()
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(description="01 后台视频分析")
    ...
    started = time.time()
    logger.info(
        "video_runner start task=%s platform=%s url=%s",
        args.task_id,
        args.platform,
        args.origin_url[:160],
    )
    try:
        ...
        logger.info("video_runner done ok=%s elapsed=%.1fs", ok, time.time() - started)
        return 0 if ok else 1
    except Exception as exc:
        logger.exception("video_runner 顶层失败")
        try:
