#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""灌入登录账号索引 hermes_xa_login_info。

用法：
  python scripts/login_info/seed_es.py --recreate
"""
...
INDEX_NAME = "hermes_xa_login_info"
HERE = Path(__file__).resolve().parent
REPO_ROOT = HERE.parents[1]
SEED_PATH = HERE / "login_info_seed.json"
MAPPING_PATH = (
    REPO_ROOT
    / "clients"
    / "hermes-xa"
...
def _es(
    ...
    if not host:
        raise RuntimeError("未配置 ES host")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="灌入登录账号")
    ...
    lines = []
    for doc in docs:
        doc_id = str(doc.get("user_id") or "").strip()
        if not doc_id:
            raise ValueError(f"缺少 user_id: {doc}")
    ...
    if result.get("errors"):
        print("bulk 有错误:", json.dumps(result, ensure_ascii=False)[:500], file=sys.stderr)
        return 1
    print("完成")
    return 0
