"""02 账号扩建：步骤3主页+发文完成后的图片资产入库（复用 image_pipeline）。

主路径：Skill 要求 Agent 执行
  python -m image_pipeline.run --task-id <taskId> --force-analyze
Hook 仅在 finalize / session_end 兜底，禁止进 post_tool 热路径。
"""
...
def run_image_pipeline_for_expand(
    task_id: str,
    *,
    ...
    if timeout_sec is None or timeout_sec <= 0:
        try:
            ...
            logger.warning("02 图片管线失败 task=%s: %s", task_id, exc)
            return None
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            try:
                ...
                logger.warning(
                    "02 图片管线等待超时(%ss)，放弃阻塞 task=%s（不 fail 任务）",
                    timeout_sec,
                    task_id,
                )
                return None
    except Exception as exc:
        logger.warning("02 图片管线异常 task=%s: %s", task_id, exc)
        return None
