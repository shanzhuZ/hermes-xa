...
- **图片流分析（硬约束，不可跳过）**：
  1. 对每个有 `avatar_url` / 头像图的种子账号，**必须各调用 1 次** `mcp_ocr_perform_ocr`（`language=chi_sim` 或 `eng`）+ **1 次** `vision_analyze`（或 `mcp_vision_analyze`），传入该头像 URL。
  2. **禁止**用常识/名人知识编造图片对比结论；3.2 表格只能基于上述工具返回。
  3. `collect_images` 仅表示系统已存图，**不等于**已完成 Vision；**禁止**以「图片已入库」为由跳过本步工具调用。
  4. 未完成全部头像的 OCR+Vision 之前，**禁止**输出三节终稿。
...
- 步骤5 必须为每一个候选账号与种子账号展示每种分析流（文本流、图片流）的详细结果；图片流必须先有 OCR/Vision 工具结果。
- 最终只输出报告三节内容；终稿**必须以** `一、账号基础信息`（或 `## 一、账号基础信息`）开头，禁止前置进度句/图片管线元叙述。
- **系统会拒绝**在 Vision 未完成时落库三节终稿；被拒后须补调 `vision_analyze` 再输出终稿。
- 所有流程步骤必须全部使用简体中文
...
**步骤4**：归纳发文风格与领域。
**步骤5**：同步执行文字流分析与图片流分析；图片流必须对每个头像真实调用 OCR + `vision_analyze`。
...
- ❌ 以「图片已入库 / collect_images / 名人常识」跳过 OCR/Vision，或无工具结果写 3.2 图片流表。
