...
- **步骤11 第三章顺序**：先写各「### 观点N」，后写「### 视频观察」。每条观点单独一个小节；「发文作证」每条证据单独一行，格式为 `1. 日期+平台+发文称「原文摘录」`（**禁止**写「链接：」「链接未获取」及任何 URL）。**原文摘录加长（硬约束）**：短帖（约 ≤120 字）尽量全文照录；长帖取与观点相关的**连续片段**，每条摘录不少于约 **40～80 字**（标点计入），禁止只写标题式短句（如仅「湖南長沙解放西, 毛澤東的傳人當街吃雞事件」这类过短一行）。**陕西重点标注（硬约束）**：原文涉及陕西（含繁体「陝西」及「西安/咸阳」等明确陕西地市）时，须用成对标签包裹相关地名或含地名的摘录片段：`[重点地域:陕西]…[/重点地域]`（如「[重点地域:陕西]陝西公車司機玩弄祖國的花朵好開心[/重点地域]」），便于前端/Word 高亮；无陕西则不套该标签。**发文作证条数（库内有帖则加厚）**：每条观点在相关发文够用时写 **3～5 条**（不同日期/不同帖，禁止同一帖复制多条）；仅能找到 1～2 条时如实写够并注明「本观点相关发文仅见 N 条」，**禁止编造**。**多平台硬约束**：步骤7已入库发文的**每一个**平台（如 X 与 YouTube）都必须在第三章观点作证中出现至少 1 条证据；禁止只写种子平台、把其它已采平台只塞进「视频观察」。同题镜像内容可在同一观点下分列两平台各若干条。有平台视频分析成功时必写「### 视频观察」：分析文本**同时**含涉华因素（含讽刺/隐喻）**且**能识别到较准确城市/地点 → 可正常展开；若**缺少地点**或**缺少涉华**（含「无法识别」「未见明显涉华因素」）→ **仅用 2～3 句**概括画面即可。禁止逐帧复述、禁止【地点】【涉华】长文、禁止用视频段落压过文本发文观点、禁止在观点之后再另起一段「视频观察：」重复叙述
...
| 9  | 文本流：结合**各个已采发文平台**归纳观点与涉华，作证摘录加长、不写链接；涉陕套 `[重点地域:陕西]`；禁止只写单一平台 |
...
### 观点1：一句话概括该观点
发文作证：
1. yyyy年mm月dd日在XX平台发文称「较长原文摘录（短帖尽量全文；长帖连续片段约 40～80 字以上）」
2. yyyy年mm月dd日在YY平台发文称「……」
3. yyyy年mm月dd日在XX平台发文称「[重点地域:陕西]含陕西地名的摘录片段[/重点地域]……」（仅涉陕时套标签）
4. yyyy年mm月dd日在XX平台发文称「……」
5. yyyy年mm月dd日在YY平台发文称「……」
（禁止写「链接：」/URL/「链接未获取」；库内够用写 3～5 条；不够则写尽并注明「本观点相关发文仅见 N 条」）
...
（禁止每条观点只列 1 条就收束——有帖就必须尽量加厚作证；禁止摘录过短）
...
- ❌ 发文作证写「链接：」/URL/「链接未获取」（本章已取消链接）
- ❌ 发文作证摘录过短（仅标题式一两句，未达约 40 字且非短帖全文）
- ❌ 原文含陕西/陝西等地名却未套 `[重点地域:陕西]…[/重点地域]`
