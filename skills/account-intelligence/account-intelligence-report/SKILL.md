...
- **步骤11** 须含「五、个人画像情况」「六、综合研判与核查情况」；可与「四、核查思路」内容重叠，禁止空章、禁止一两句话敷衍
- **步骤11 Twitter/X 主页表述（硬约束）**：该号给别人点过的赞写「点赞他人推文 N 次」，被列入公开列表写「被列入清单 N 次」；**禁止**写成「获赞」「收到赞」「帖子获赞」。**终稿正文禁止出现任何英文字段名**（如 favourites_count、listed_count）及括号参数标注。无账号级「帖子总获赞」；要写帖子互动须基于步骤7入库发文抽样汇总并注明非全量
- **步骤11 报告标签（必写，落库用）**：通读**整篇六章**后，在终稿**最末尾**按下方骨架输出 `[报告标签]…[/报告标签]` 块（系统会剥掉，用户正文不展示）。块内写 1～8 个中文主题标签，逗号分隔；有相关内容才打标，禁止编造；可自拟（如 `地理位置`、`组织关联`），每项 2～6 字。**禁止删掉该块、禁止只写开标签不写闭标签、禁止插在章节中间**。**注意：`collect-rules.yaml` 的 section6 亦要求同块，不可省略**
- **步骤11 第三章顺序**：先写各「### 观点N」，后写「### 视频观察」。每条观点单独一个小节；「发文作证」每条证据单独一行，格式为 `1. 日期+平台+原文；链接：原文URL`（有 `content_url` 用库内链接，否则拼平台永久链接：X→`https://x.com/{handle}/status/{id}`，YouTube→`https://www.youtube.com/watch?v={id}`；无链接才可省略「链接：」并注明「链接未获取」）。**发文作证条数（库内有帖则加厚）**：每条观点在相关发文够用时写 **3～5 条**（不同日期/不同帖，禁止同一帖复制多条）；仅能找到 1～2 条时如实写够并注明「本观点相关发文仅见 N 条」，**禁止编造**。**多平台硬约束**：步骤7已入库发文的**每一个**平台（如 X 与 YouTube）都必须在第三章观点作证中出现至少 1 条证据；禁止只写种子平台、把其它已采平台只塞进「视频观察」。同题镜像内容可在同一观点下分列两平台各若干条。有平台视频分析成功时必写「### 视频观察」：分析文本**同时**含涉华因素（含讽刺/隐喻）**且**能识别到较准确城市/地点 → 可正常展开；若**缺少地点**或**缺少涉华**（含「无法识别」「未见明显涉华因素」）→ **仅用 2～3 句**概括画面即可。禁止逐帧复述、禁止【地点】【涉华】长文、禁止用视频段落压过文本发文观点、禁止在观点之后再另起一段「视频观察：」重复叙述
...
| 9  | 文本流：结合**各个已采发文平台**归纳观点与涉华，每条作证须带原文链接；禁止只写单一平台 |
| 10 | 文本流： 结合 各个平台的账号发文 分析真实姓名、年龄、籍贯、常住地、活动城市、生活习惯、教育经历、工作经历、对华态度、电话邮箱码值、社交、亲友、同事等三个圈层关系 |
| 11 | 一次输出账号画像报告：一、账号基本信息 二、账号全网关联账号 三、账号网络活动情况（含视频观察若有） 四、核查思路 五、个人画像情况 六、综合研判与核查情况 |
...
| 禁空等 | 发文工具被门禁拦截时：**禁止结束会话空等**；步骤4未完则继续主页，步骤4已完则**保持会话**等下一轮 `step7_posts` 放行后立刻采发文。**禁止**写「等待系统完成 4.2/4.3」后 done |
...
**步骤7执行硬约束（本回合优先）**：
1. 打开步骤7后，**本回合优先**对上下文「尚未尝试发文」列表逐平台调工具，禁止先写步骤8～11、禁止结束会话空等、禁止「等待系统」收尾。
...
5. 若刚写完 4.1 文本核验：检查编排是否已是 `step7_posts`；是则**同回合或下一回合立刻**调发文，勿结束会话。

## 步骤 11 输出骨架

**硬约束**：终稿正文**必须以** `## 一、账号基本信息` 或 `一、账号基本信息` 开头（前导空白除外）；**禁止**在第一节之前写进度句、步骤号、图片管线、task_id 等任何元叙述。
**排版**：严格按下面骨架的换行；章题/小节标题单独成行；小节之间空一行。关联 YouTube 等放「二」不要塞进「一」的同一段。

```
## 一、账号基本信息
【平台·@handle】一句话点明平台、handle、昵称。

### 1.1 主页信息
昵称、用户ID、注册时间、认证/蓝V/保护状态、粉丝/关注/发文/媒体帖、点赞他人推文次数、被列入清单次数、简介、位置、外链、头像与横幅要点（无个人影像则写明）。
示例写法：「粉丝 34856，关注 0，发文 13333，媒体帖 13293，点赞他人推文 4 次，被列入清单 146 次。」（禁止在正文夹带英文字段名或括号参数）
YouTube：写订阅、视频数、总播放等中文统计。本小节可 1～2 段，禁止与 1.2 糊成一段。

### 1.2 发文特征
最早可见发帖时间、发帖频率、活跃时段、发文语种、固定格式（如结尾表情）、媒体占比、内容题材概要。只写本平台发文习惯；跨平台账号放到第二章。

## 二、账号全网关联账号
### 2.1 强关联账号
每个强关联账号单独一段：平台、handle/ID、关键证据（昵称、外链互指、同日同题等）。禁止把弱关联、第三方转载写进本小节。

### 2.2 弱关联与排除
同名巧合、第三方转载/引用，各用一段说明「为何不是本人账号」。无则写「未发现弱关联候选」。
...
### 观点1：一句话概括该观点
发文作证：
1. yyyy年mm月dd日在XX平台发文称「原文或准确转述」；链接：https://……
2. yyyy年mm月dd日在YY平台发文称「……」；链接：https://……
3. yyyy年mm月dd日在XX平台发文称「……」；链接：https://……
4. yyyy年mm月dd日在XX平台发文称「……」；链接：https://……
5. yyyy年mm月dd日在YY平台发文称「……」；链接：https://……
（库内该观点相关发文够用时写 3～5 条；不够则写尽并注明「本观点相关发文仅见 N 条」）
...
2. ……；链接：https://……
3. ……；链接：https://……
（观点N 均按此格式，每条观点一个小节，禁止观点连写）
（多平台：步骤7已入库的每个平台，整章观点作证合计至少出现 1 条；禁止整章只写单一平台）
（禁止每条观点只列 1 条就收束——有帖就必须尽量加厚作证）

### 视频观察
（有视频分析时必写，且必须放在全部观点之后）
按平台分行，每平台 2～3 句；同时含涉华与较准确城市/地点才可略写长。缺地点或无涉华 → 只概括画面，禁止【地点】【涉华】专栏、禁止逐帧、禁止重复第二段「视频观察：」。

## 四、核查思路
每个点位单独一个小节，先给结论再给依据；未获取也要单独成行，禁止把十几个点位挤成一段。
...
## 五、个人画像情况
用「一是……。二是……。三是……。」连续编号叙述，**至少写到「五是」**（有材料则写到六～八是）。**每一「是」单独成段**，每段不少于约 80～120 字，仍用叙述句，不要改成表格。须覆盖：身份与职业推断、活动范围/常住与出行、议题与对华态度、账号运行方式（个人/团队）、社交圈层与生活习惯等；材料不足的点也要写「依据不足，仅能……」。可与第四章重叠，禁止空章，禁止整章只有两三句。

## 六、综合研判与核查情况
结合以上信息作初步研判（**不少于 2～4 段、合计约 400 字以上**）：先概括此人/此号是谁、主要在哪些平台活动、核心议题与对华态度；再写可信度与证据缺口（哪些点已坐实、哪些仅推断）；最后点明风险或关注方向。禁止用一两句话代替本章。

建议后续核查（**不少于 4 步**，每步写清目的与可操作线索，不要只写「继续关注」）：
第一步：……
第二步：……
第三步：……
第四步：……
...
[报告标签]涉华,视频搬运,匿名运营[/报告标签]
```

**标签块说明**：上行为格式示例，输出时须**通读六章后**把示例标签**全部替换**为本文归纳的真实标签（1～8 个，逗号分隔）；**必须保留** `[报告标签]` 与 `[/报告标签]` 成对标记，禁止省略或删块。

## 禁止收尾示例

- ❌ 近期推文主题归纳（无原文）
- ❌ 「如果你想进一步了解…」
- ❌ 结尾写「（终稿完）」/报告完毕/执行完毕/数据来源等
- ❌ 把一～六章糊成各自一大段、不换行、不用 `###` 小节
- ❌ 第三章先写长篇视频观察再塞观点，或观点与发文作证挤在同一行
- ❌ 发文作证无原文链接（有 content_url / 可拼永久链接却不写）
- ❌ 库内该观点相关发文明显够用，但每条观点只列 1 条发文作证
- ❌ 步骤7已采多平台发文，但第三章观点作证只写单一平台（如只有 X、没有 YouTube）
- ❌ 第五章只有两三句或不足五「是」；第六章研判不足两段或后续核查不足四步
- ❌ 第一节仍写「profile信息」（须用「主页信息」）
- ❌ 步骤 7.5 / 图片管线 / 未部署 / 继续步骤 8（任何元叙述写进终稿）
- ❌ 终稿不以「一、账号基本信息」开头（前缀进度句、管线摘要一律禁止）
- ❌ 终稿末尾缺少 `[报告标签]…[/报告标签]` 块，或只写开标签不写闭标签
- ❌ Twitter/X 把「点赞他人推文」写成「获赞」「收到赞」「帖子获赞」
- ❌ 终稿正文出现 favourites_count / listed_count 等英文字段名或「（favourites_count）」类括号参数
