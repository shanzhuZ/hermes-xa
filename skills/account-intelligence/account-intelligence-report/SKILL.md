...
- **步骤11** 必须按一～六章结构输出（章节齐全）。**排版硬约束**（禁止整章糊成一段）：章题单独成行；章与章之间空一行；章内按小节换行；小节标题用 `###` 且单独成行；同一小节内用 1～3 段叙述即可，**禁止**把整章所有内容连成一大段。**禁止**在终稿末尾写「（终稿完）/报告完毕/执行完毕」
- **步骤11** 须含「五、个人画像情况」「六、综合研判与核查情况」；可与「四、核查思路」内容重叠，禁止空章
- **步骤11 第三章顺序**：先写各「### 观点N」，后写「### 视频观察」。每条观点单独一个小节；「发文作证」每条证据单独一行，格式为 `1. 日期+平台+原文；链接：原文URL`（有 `content_url` 用库内链接，否则拼平台永久链接：X→`https://x.com/{handle}/status/{id}`，YouTube→`https://www.youtube.com/watch?v={id}`；无链接才可省略「链接：」并注明「链接未获取」）。**多平台硬约束**：步骤7已入库发文的**每一个**平台（如 X 与 YouTube）都必须在第三章观点作证中出现至少 1 条证据；禁止只写种子平台、把其它已采平台只塞进「视频观察」。同题镜像内容可在同一观点下分列两平台各 1 条。有平台视频分析成功时必写「### 视频观察」：分析文本**同时**含涉华因素（含讽刺/隐喻）**且**能识别到较准确城市/地点 → 可正常展开；若**缺少地点**或**缺少涉华**（含「无法识别」「未见明显涉华因素」）→ **仅用 2～3 句**概括画面即可。禁止逐帧复述、禁止【地点】【涉华】长文、禁止用视频段落压过文本发文观点、禁止在观点之后再另起一段「视频观察：」重复叙述
...
## 五、个人画像情况
用「一是……。二是……。三是……。四是……。」叙述，每「是」可单独成段（仍用叙述句，不要改成表格）。可与第四章重叠，禁止空章。
